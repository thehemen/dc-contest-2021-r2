...
int tgcat_detect_language(const struct TelegramChannelInfo *channel_info,
  ...
  size_t i;
  for (i = 0; i < channel_info->recent_post_count; i++) {
    if (strlen(channel_info->recent_posts[i].text) > text_len) {
      text = channel_info->recent_posts[i].text;
      text_len = strlen(channel_info->recent_posts[i].text);
    }
  }
